...
class MyEnvironmentNode(Node):
	...
	def __init__(self):
		...
		self.contact_sensor_subscription = self.create_subscription(ContactsState, '/contact_sensor/wrist_3_link', self.contact_state_callback, 1)
	...
	def on_timer_transformation(self):
		...
		try:
			...
			trans = self.tf_buffer.lookup_transform(self.reference_frame, self.child_frame, now) # This calculate the position of the wrist_3_link w.r.t. world frame
	...
	def contact_state_callback(self, msg):
		...
		home_point_msg.time_from_start = Duration(sec=5)
	...
	def action_generator_sample(self):
		...
		angle_j_1 = random.uniform( -3, 3)   # values in degrees [   0,  360]
		angle_j_2 = random.uniform(-6.28, 6.28)	 # values in degrees [- 35,  35]
		angle_j_3 = random.uniform(-6.28, 6.28)  # values in degrees [- 45,  45]
		angle_j_4 = random.uniform(-6.28, 6.28)	 # values in degrees [- 45,  45]
		angle_j_5 = random.uniform(-6.28, 6.28)	 # values in degrees [- 45,  45]
		angle_j_6 = random.uniform(-3, 3)  # values in degrees [- 45,  45]
	...
	def action_step_service(self, action_values):
		...
		point_msg.time_from_start = Duration(sec=3)


		joint_names = ['shoulder_1_joint','shoulder_2_joint','elbow_joint','wrist_1_joint','wrist_2_joint','wrist_3_joint']
        
		goal_msg = FollowJointTrajectory.Goal()
		goal_msg.goal_time_tolerance    = Duration(sec=2) 	# goal_time_tolerance allows some leeway in time, so that the trajectory goal can still
	...
	def distance_calculator(self):
		...
		# i.e. Calculate the eucladian distance between the wrist_3_link(end effector) and  shpere (target point)
	...
	def reward_calculator(self):
		...
		if distance <= 0.05:
			...
			if self.collision_flag:
				self.get_logger().info('Collision wrist_3_link')
				reward_d = -100
				done = 'Collision'
				self.collision_flag = False 
			else:
