...
class MyRLEnvironmentNode(Node):
	...
	def contact_state_callback(self, msg):
		if msg.states:  # lista non vuota → collisione avvenuta
	...
	def reset_environment_request(self):
		...
		home_point_msg.time_from_start = Duration(seconds=5).to_msg()
	...
	def action_step_service(self, action_values):
		...
		point_msg.time_from_start = Duration(seconds=3.0).to_msg() # be careful about this time 
	...
	def generate_action_funct(self):
		...
		angle_j_1 = random.uniform( 0, 3.14)   
		angle_j_2 = random.uniform(-0.7, 0.7)	 
		angle_j_3 = random.uniform(-0.7, 0.7) 
		angle_j_4 = random.uniform(-0.7, 0.7)	 
		angle_j_5 = random.uniform(-1.5, 1.5)	
		angle_j_6 = random.uniform(0, 2.15)  
	...
	def calculate_reward_funct(self):
		...
		# i.e. Calculate the euclidean distance between the wrist_3_link (end effector) and sphere (target point)
		# and each timestep the robot receives -1 but if it reaches the goal (distance < 0.05) receives +10

		if self.collision_flag:
			done = True
			reward_d = -100
			self.collision_flag = False  # reset flag
